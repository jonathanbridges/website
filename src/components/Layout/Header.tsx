...
function themeButtonClass(isActive: boolean) {
  return `cursor-pointer rounded px-2 py-1 text-xs uppercase tracking-wide transition-colors transition-opacity duration-200 ${
    isActive
      ? "bg-[var(--color-accent)] text-[var(--color-bg)] hover:opacity-80"
      : "text-muted hover:text-[var(--color-accent)]"
  }`;
}
    ...
    >
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-6 md:px-12">
        <button
          ...
          className={`${eyebrowPrimary} cursor-pointer tracking-[0.35em] transition-opacity hover:opacity-60 ${
            activeSection === "home" ? "text-[var(--color-accent)]" : ""
          }`}
        >
        ...
        <div className="flex items-center gap-3">
          <div className="hidden items-center gap-1 sm:flex" role="group" aria-label="Theme">
            {(["light", "dark", "geocities"] as Theme[]).map((t) => (
              <button
                ...
                className={themeButtonClass(theme === t)}
              >
                {THEME_LABELS[t]}
              </button>
            ))}
          ...
          <button
            type="button"
            onClick={cycleTheme}
            className={`${themeButtonClass(false)} sm:hidden`}
            ...
            {NAV_SECTIONS.map(({ id, label }) => (
              <li key={id}>
                <button
                  type="button"
                  onClick={() => handleNav(id)}
                  className={`w-full text-left ${navLinkClass(activeSection === id)}`}
                >
                  {label}
                </button>
              </li>
